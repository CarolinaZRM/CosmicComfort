...
exports.createFakeCall = async (req, res) => {
    ...
    try {
      const requiredFields = ['user_id', 'caller_name', 'call_time', 'ringtone', 'ring_name', 'profile_picture']
...
exports.getFakeCallByUserId = async (req,res) => {
    const user_id = req.params.id;
    try {
        const fakeCall = await fakeCallDao.getFakeCallByUserId(user_id);
        if (!fakeCall) return res.status(404).json({ message: 'Fake call for this user not found' });
        res.status(200).json(fakeCall);
    } catch (err) {
        res.status(500).json({ message: "Error getting fake call", err: err.message });
    }

};

exports.updateFakeCallByUserId = async (req, res) => {
    ...
    try {
      ...
      if (!json_user) {
          return res.status(404).json({ message: 'User not found, fake call not updated' });
      }

      // Find fake call of user
      const fakeCall = await fakeCallDao.getFakeCallByUserId(userId);
      if (!fakeCall) {
          return res.status(404).json({ message: 'Fake call not found for this user' });
      }
      
      fakeCallId = fakeCall._id;
      
      // Update the fake call via the DAO
      const updatedFakeCall = await fakeCallDao.updateFakeCall(fakeCallId, updateData);  
      if (!updatedFakeCall) {
          return res.status(404).json({ message: 'Fake call not found' });
      }
      res.status(200).json({ message: 'Fake call updated successfully', fakeCall: updatedFakeCall });
